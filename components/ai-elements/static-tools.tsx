...
import { SubAgent, SubAgentToolNames, SubAgentTools, SubAgentUIMessage, SubAgentUITools } from "@/lib/research/agents/sub.agents";
    ...
    <Collapsible defaultOpen={false} className={cn("group/tool", className)} {...props}>
      ...
      <CollapsibleContent>
        <div className="px-4 py-3 ml-7 border-l border-border/50">
          <p className="text-sm text-muted-foreground">{input.query}</p>
        </div>
      </CollapsibleContent>
    </Collapsible>
  );
...
export function StaticToolRenderer({ part }: StaticToolPartProps) {
  ...
  switch (part.type) {
    ...
    case "tool-browseWeb":
      return (
        <BrowseWebTool part={part} />
      );
    case "tool-calculate":
      return (
        <CalculateTool part={part} />
      );
    case "tool-delay":
      return (
        <DelayTool part={part} />
    ...
    case "tool-getCurrentDateTime":
      return (
        <GetCurrentDateTimeTool part={part} />
      );
    case "tool-sendEmail":
      return (
        <SendEmailTool part={part} />
      );
    default:
