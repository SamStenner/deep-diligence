...
import type { Project, ProjectResearch, Subagent } from "@/lib/data/schema";
import {
  type AgentIcon,
  subAgentPropertiesRegistry,
} from "@/lib/research/agents/agent.properties";
import type { AgentUIMessage } from "@/lib/research/agents/orchestrator";
import type {
  SubAgentName,
  SubAgentUIMessage,
} from "@/lib/research/agents/types";
...
function parseResearch(research: AgentUIMessage | null) {
  const results = research?.parts.filter((p) => isToolUIPart(p));
  const doneResult = results?.find(
    (r) => r.type === "tool-done" && r.state === "output-available",
  );
  const outputResults = results?.filter((r) => r.state === "output-available");
  const latestPlan = outputResults?.findLast(
    (r) => r.type === "tool-updatePlan",
  )?.input.plan;

  // Extract all process steps in order
  const processSteps: ProcessStep[] | null =
    results
      ?.filter((r) =>
        [
          "tool-updatePlan",
  ...
  return {
    plan: latestPlan,
    processSteps,
    status: (doneResult ? "completed" : "in_progress") as
      | "completed"
      | "in_progress",
...
function SubAgentView({ messages }: { messages: SubAgentUIMessage[] }) {
  return (
    <div className="space-y-3 bg-card p-6 rounded-xl h-full">
      {messages.map((message) => (
        <Content key={message.id} parts={message.parts} />
      ))}
    </div>
  );
}
...
const stepConfig: Record<
  ProcessStep["type"],
  { icon: AgentIcon; label: string; getDescription: (input: unknown) => string }
> = {
  updatePlan: {
    icon: "ClipboardList",
    label: "Update Plan",
  ...
  return (
    <div className="space-y-0">
      {processSteps.map((step, index) => {
        ...
        const spawnedAgents =
          step.type === "spawnAgent"
            ? (
              step.input as {
                subAgents: Array<{ subAgent: string; prompt: string }>;
              }
            ).subAgents
            : [];
              ...
              {step.type === "spawnAgent" && spawnedAgents.length > 0 && (
                <div className="mt-2 flex flex-wrap gap-1.5">
                  {spawnedAgents.map((agent, agentIndex) => {
                    const agentProps =
                      subAgentPropertiesRegistry[
                      agent.subAgent as keyof typeof subAgentPropertiesRegistry
                      ];
                    return (
                      <div
                        key={agentIndex}
...
export type SubAgentMessageData = {
  id: string;
  name: SubAgentName;
...
export function ProjectView({
  ...
  const report = parseResearch(research);
  const [selectedTab, setSelectedTab] = useState<string | null>(null);
  const [mounted, setMounted] = useState(false);
  const selected = subagents.find((a) => a.id === selectedTab);
  const subAgentName = selected?.name
  const subAgentProperties = subAgentName
    ? subAgentPropertiesRegistry[subAgentName]
    : null;
  const images = subAgentName
    ? (subAgentImages[subAgentName] ?? subAgentImages.default)
    : null;
  const { resolvedTheme } = useTheme();

  useEffect(() => setMounted(true), []);

  const messages = selected?.messages.filter((m) => m !== null).map((m) => m.message!) ?? []
            ...
            <SidebarGroupContent>
              <SidebarMenu>
                {subagents.map((agent) => {
                  ...
                  const subagent = subAgentPropertiesRegistry[subAgentName];
                  return (
      ...
      <SidebarInset className="flex flex-col bg-muted/30">
              ...
              <DropdownMenuContent align="end">
                ...
                >
                  ...
                  >
                    ...
                    />
                    <path
                      d="M7 13.5L8.5 18l1.5-4.5L11.5 18 13 13.5"
                      fill="none"
                      stroke="white"
                      strokeWidth="1.5"
                      strokeLinecap="round"
        ...
        {selectedTab === null ? (
            ...
            <div className="space-y-6">
              ...
              <Tabs defaultValue="report" className="w-full">
                ...
                <TabsContent value="process">
                  {report.processSteps && (
                    <div className="p-6 rounded-xl bg-card">
                      <ProcessTimeline processSteps={report.processSteps} />
                    </div>
                  )}
                </TabsContent>
              </Tabs>
          ...
          <BackdropCard
            ...
            imageSrc={subImage!}
            rootClassName="p-6"
            className="p-10 max-w-4xl 2xl:max-w-6xl"
            imageClassName=""
          >
            <div className="space-y-6">
              ...
              <SubAgentView messages={messages} />
            </div>
          </BackdropCard>
        ) : null}
      </SidebarInset>
...
const subAgentImages: Partial<Record<SubAgentName, ImageTheme>> & {
  default: ImageTheme;
} = {
  general: {
...
function Icon({
  ...
  className,
  ...props
}: HTMLAttributes<HTMLOrSVGElement> & { icon: AgentIcon }): React.ReactNode {
